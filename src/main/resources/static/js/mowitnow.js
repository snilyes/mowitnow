	...
	zone.event('send', function(files) {
		files.each(function(file) {
			file.readData(function(str) {
				...
				$("#instructions-error").empty().append(
						"<strong>Erreur</strong>: " + e).show();
			}, 'text');
		});
	});
}
...
function connect() {
	...
	stompClient.connect({}, function(frame) {
		...
		stompClient.subscribe('/mowers/error', function(data) {
			$("#instructions-error").empty().append(
					"<strong>Erreur</strong>: " + data.body).show();
			receive();
		});
...
function draw() {
	...
	$(
			'<canvas id="canvas" width="' + (20 + cellWith * width)
					+ '" height="' + (20 + cellWith * height) + '"></canvas>')
	...
	for (var i = 0; i < width; i++) {
		$("canvas").drawText({
			...
			strokeStyle : "#2a5",
			strokeWidth : 1,
			x : 10 + cellWith * i + (cellWith / 2),
			y : 10 + cellWith * (height),
			text : i + 1
		});
	}
	...
	for (var j = 0; j < height; j++) {
		$("canvas").drawText({
			...
			strokeWidth : 1,
			x : 10,
			y : cellWith * (height - j - 1) + (cellWith / 2),
			text : j + 1
		});
	}
...
function result() {
	...
	for ( var i in monitor.mowers) {
		...
		$("#demo-result").append(
				'<p>La tondeuse ' + increment(i) + ' => ('
						+ increment(mower.cell.position.x) + ','
						+ increment(mower.cell.position.y) + ','
						+ mower.orientation + ')</p>');
	}
	$("#demo-result").show();
}
