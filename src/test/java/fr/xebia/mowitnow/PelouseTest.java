  ...
  public void pelouse(final int largeur, final int longueur) {
    ...
    for (int x = 0; x < largeur; x++) {
      for (int y = 0; y < longueur; y++) {
        ...
        assertVoisin(pelouse, EST, cellule.getPosition(), new Position(x + 1, y));
        assertVoisin(pelouse, WEST, cellule.getPosition(), new Position(x - 1, y));
        assertVoisin(pelouse, NORD, cellule.getPosition(), new Position(x, y + 1));
        assertVoisin(pelouse, SUD, cellule.getPosition(), new Position(x, y - 1));
      }
    }
  }

  public void assertVoisin(final Pelouse pelouse, final Orientation orientation,
      final Position position, final Position positionAttendue) {
    Cellule cellule = pelouse.cellule(position);
    Cellule actuelle = cellule.getVoisin(orientation);
    Cellule attendue = null;
    if (entre(positionAttendue.getX(), 0, pelouse.getLargeur())
        && entre(positionAttendue.getY(), 0, pelouse.getLongueur())) {
